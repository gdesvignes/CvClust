...
# Calling convention:
#
# NBPP_presto_search.py fil_file working_dir
#
...
institution           = "NRAOCV" 
base_output_directory = "/data/NBPP/results/"
...
rfifind_chunk_time    = 2**14 * 0.00006  # ~2.1 sec for dt = 64us 
...
# Create our de-dispersion plans (for NBPP)
...
if (1):
    ...
    ddplans.append(dedisp_plan(   0.0,   0.3,     212,     1,       96,       1))
    ddplans.append(dedisp_plan(  63.7,   0.5,      75,     1,       96,       1))
    ddplans.append(dedisp_plan( 101.2,   1.0,      89,     1,       96,       2))
    ddplans.append(dedisp_plan( 190.2,   3.0,      81,     1,       96,       4))
    ddplans.append(dedisp_plan( 433.2,   5.0,     155,     1,       96,       8))
else: # faster option that sacrifices a small amount of time resolution at the lowest DMs
    # The values here are:       lodm dmstep dms/call #calls #subbands downsamp
    ddplans.append(dedisp_plan(   0.0,   0.5,      22,    21,       96,       1))
    ddplans.append(dedisp_plan( 231.0,   0.5,      24,     6,       96,       2))
    ddplans.append(dedisp_plan( 303.0,   1.0,      24,    11,       96,       4))
    ddplans.append(dedisp_plan( 567.0,   3.0,      24,     7,       96,       8))
    
def main_process(nbpp_filenm_long, cpu_id, box):
